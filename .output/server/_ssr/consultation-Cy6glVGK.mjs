...
import { t as notifyConsultation } from "./email-pwtdYqxW.mjs";
import { i as isSpam, t as ConsultationSchema } from "./validations-XDGyLUuA.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/consultation-Cy6glVGK.js
/**
...
var submitConsultation = createServerFn({ method: "POST" }).validator((d) => ConsultationSchema.parse(d)).handler(submitConsultation_createServerFn_handler, async ({ data }) => {
	const input = data;
	if (!rateLimit(`consult:${getRequestIP$1({ xForwardedFor: true }) ?? "unknown"}`, 8, 6e4)) throw new Error("Too many requests. Please try again in a minute.");
